...
def _safe_json_loads(s: str, default: dict | list | None = None):
    """
    Parse LLM JSON output robustly. Attempts simple repairs when needed.
    ...
    if m:
        ...
        try:
            ...
            raw = inner  # keep working on inner

    # Quick repairs: trailing commas ,] ,}
...
class HybridRAG:
    ...
    # =================== Build majors from PDF =====================

    def build_majors_from_pdf(
        self,
        majors_pdf_path: str = "data/majors.pdf",
        out_json: str = "extracted_majors.json"
    ) -> List[MajorProfile]:
        """
        Extract majors & sample courses from majors.pdf (any language).
        Keep prompts in English; translate each major name to English for embedding.
        Robust to messy LLM JSON; always writes a JSON file (even empty) so app won't re-build repeatedly.
        """
        if not os.path.exists(majors_pdf_path):
            print(f"[majors] File not found: {majors_pdf_path}")
        ...
        # 1) Read PDF text
        loader = PyPDFLoader(majors_pdf_path)
        pages = loader.load()
        full_text = "\n".join(p.page_content for p in pages)

        # 2) Try LLM JSON extraction
        majors: List[MajorProfile] = []
        try:
            majors = self._extract_majors_via_llm(full_text)
        except Exception as e:
            print(f"[majors] LLM extraction error (continuing with fallback): {e}")

        # 3) Fallback heuristic parse if too few
        if len(majors) < 3:
            try:
                heuristic = self._extract_majors_heuristic(full_text)
                seen = {m.original_name for m in majors}
                majors.extend([h for h in heuristic if h.original_name not in seen])
            except Exception as e:
                print(f"[majors] Heuristic extraction error: {e}")

        # 4) Ensure English names for all (translate originals when missing)
        majors = self._ensure_english_names(majors)

        # 5) Deduplicate by english_name
        uniq: Dict[str, MajorProfile] = {}
        for m in majors:
            on = (m.original_name or "").strip()
            en = (m.english_name or "").strip()
            if not en:
                continue
            if en not in uniq:
                uniq[en] = MajorProfile(
                    original_name=on,
                    english_name=en,
                    keywords=[kw.strip() for kw in (m.keywords or []) if kw.strip()],
                    sample_courses=[c.strip("•- \u2022\t") for c in (m.sample_courses or []) if c.strip()],
                    source=os.path.basename(majors_pdf_path)
                )
            else:
                u = uniq[en]
                u.keywords = list({*u.keywords, *m.keywords})
                u.sample_courses = list({*u.sample_courses, *m.sample_courses})

        self.majors = list(uniq.values())

        # 6) Pre-embed using ENGLISH info
        self._major_vectors = self._embed_majors(self.majors)
        self._name_map = {m.english_name: m.original_name for m in self.majors}

        # 7) Save (always write something)
        with open(out_json, "w", encoding="utf-8") as f:
            json.dump([asdict(m) for m in self.majors], f, ensure_ascii=False, indent=2)

        print(f"[ok] Parsed {len(self.majors)} majors -> {out_json}")
        return self.majors
    ...
    def _extract_majors_via_llm(self, text: str) -> List[MajorProfile]:
        """
        Ask Gemini to return STRICT JSON with majors found in the document.
        Keep prompts in English and request an English translation for each major name.
        """
        schema = {
            "majors": [
                {
                    "original_name": "string (exactly as in the PDF)",
                    "english_name": "string (translated major name in English)",
                    "keywords": ["3–10 short topic keywords (any language)"],
                    "sample_courses": ["3–20 course titles (any language)"]
                }
            ]
        }
        ...
        tmpl = ChatPromptTemplate.from_messages([
            ("system",
             "You are an academic data extractor. Read the program/majors catalog text and return ONLY JSON.\n"
             "Find explicit majors (do not invent). Include the original major name exactly as in the text and an English translation.\n"
             "If a course list appears under a major, capture representative course titles."),
            ("human",
             "Return valid JSON only (no commentary), following this schema:\n{schema}\n\n"
             "Source text:\n---\n{text}\n---\n"
             "Important:\n- Keep 'original_name' exactly as in source.\n"
             "- Provide 'english_name' as a conventional English major name.\n"
             "- If unsure about a translation, choose the closest conventional academic term.\n"
             "- Do NOT include explanations outside JSON.\n"
             "JSON only:")
        ])
        prompt = tmpl.format(schema=json.dumps(schema, ensure_ascii=False), text=text)
        resp = self.llm.invoke(prompt).content

        data = _safe_json_loads(resp, default={"majors": []})
    ...
    def _extract_majors_heuristic(self, text: str) -> List[MajorProfile]:
        """
        Rough parser for cases where LLM extraction is weak.
        Looks for header-like lines and collects bullet-like lines as sample courses.
        """
        ...
        while i < len(lines):
            ...
            header_hit = (
                any(tok in line for tok in ["B.A", "B.Sc", "B.Eng", "Degree", "Major"]) or
                any(tok in line for tok in ["תואר", "חוג", "מדעים", "הנדסה"]) or
                bool(re.match(r"^[A-Z][A-Za-z &/().\-]{2,}$", line))
            )
            if not header_hit:
                i += 1
                continue

            original_name = self._clean_header(line)
            sample_courses: List[str] = []
            j = i + 1
            in_courses = False
            ...
            while j < len(lines):
                l2 = lines[j]
                next_header = (
                    any(tok in l2 for tok in ["B.A", "B.Sc", "B.Eng", "Degree", "Major"]) or
                    any(tok in l2 for tok in ["תואר", "חוג", "מדעים", "הנדסה"]) or
                    bool(re.match(r"^[A-Z][A-Za-z &/().\-]{2,}$", l2))
                )
                if next_header:
                    break
                if re.match(r"^[\-\–\—•\u2022\*]?\s*[\w\u0590-\u05FF\u0600-\u06FF].+", l2):
                    in_courses = True
                    sample_courses.append(re.sub(r"^[\-\–\—•\u2022\*]+\s*", "", l2))
                elif in_courses and not l2:
                    break
                j += 1

            majors.append(MajorProfile(
                original_name=original_name,
                english_name="",   # will be filled later
                keywords=[],
    ...
    def _clean_header(self, s: str) -> str:
        s = re.sub(r"^[\-\–\—•\u2022\* ]+", "", s).strip()
    ...
    def _ensure_english_names(self, majors: List[MajorProfile]) -> List[MajorProfile]:
        """Fill english_name for all majors using one LLM translation call."""
        need = [m for m in majors if not m.english_name.strip()]
        if not need:
            return majors

        unique_src = sorted({m.original_name for m in need if m.original_name})
        if not unique_src:
            return majors

        tmpl = ChatPromptTemplate.from_messages([
            ("system",
             "You are an academic translator. Convert the following list of program/major names into natural English major names."),
            ("human",
             "Return ONLY JSON mapping original->english, no commentary.\n"
             "Example: {\"מקצוע X\": \"Major X\", \"הנדסת מחשבים\": \"Computer Engineering\"}\n\n"
             "Original list:\n{names}\n\nJSON only:")
        ])
        prompt = tmpl.format(names=json.dumps(unique_src, ensure_ascii=False))
        resp = self.llm.invoke(prompt).content

        mapping = _safe_json_loads(resp, default={})

        out: List[MajorProfile] = []
        for m in majors:
            en = m.english_name.strip() or str(mapping.get(m.original_name, "")).strip()
            if not en:
                en = m.original_name  # last resort
            out.append(MajorProfile(
                original_name=m.original_name,
                english_name=en,
                keywords=m.keywords,
                sample_courses=m.sample_courses,
                source=m.source,
            ))
        return out
    ...
    def _embed_majors(self, majors: List[MajorProfile]) -> List[Tuple[str, List[float]]]:
        texts = []
        names = []
        for m in majors:
            profile_text = " | ".join([
